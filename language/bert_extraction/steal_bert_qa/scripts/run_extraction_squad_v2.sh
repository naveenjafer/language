...
python -m language.bert_extraction.steal_bert_qa.data_generation.preprocess_thief_dataset_squad \
  --input_path=$SQUAD_DIR/train-v2.0.json \
...
python -m language.bert_extraction.steal_bert_qa.utils.combine_qa \
  ...
  --output_path=$EXTRACTION_DATA/train-v2.0.json
...
python -m language.bert_extraction.steal_bert_qa.models.run_squad \
  ...
  --train_file=$EXTRACTION_DATA/train-v2.0.json \
  --predict_input_file=$SQUAD_DIR/dev-v2.0.json \
...
python -m language.bert_extraction.steal_bert_qa.utils.evaluate_squad_2 \
  --data_file=$SQUAD_DIR/dev-v2.0.json \
